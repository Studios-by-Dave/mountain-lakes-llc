    ...
    <>
      ...
      <Section>
        <div className="container-page grid gap-10">
          {services.map((s, i) => (
            <article key={s.slug} className={`grid gap-8 md:grid-cols-2 md:items-center ${i % 2 ? "md:[&>img]:order-2" : ""}`}>
              <img src={s.img} alt={s.title} loading="lazy" className="aspect-[4/3] w-full rounded-2xl object-cover shadow-[var(--shadow-card)]" />
              <div>
                <SectionHeading eyebrow={`0${i + 1}`} title={s.title} intro={s.blurb} />
                <Link to={`/services/${s.slug}`} className="mt-6 inline-flex items-center gap-1.5 text-sm font-semibold text-primary hover:text-primary/80">
